...
impl EditFSFileTab {
    ...
    pub fn new() -> Self {
        ...
        let mut text_input = TextInput::new(&"".to_string(), 1);
    ...
    pub fn init(&mut self, fs_file: FSFile) {
        ...
        self.category_list_state.select(Some(match fs_file {
            FSFile::Normal(_) => 0,
            FSFile::None => 1,
            FSFile::Swap => 2
        }));
...
impl ScreenTab for EditFSFileTab {
    ...
    fn handle_key_event(&mut self, key_event: crossterm::event::KeyEvent) -> Option<crate::screens::screen::ScreenAction> {
        ...
        self.text_input_visible = match self.category_list_state.selected() {
            Some(0) => true,
            _ => false
        };
